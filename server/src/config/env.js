...
export const env = {
  ...
  payment: {
    ...
    // Generic key/secret retained for backward-compat / non-Stripe
    // providers; Stripe specifically uses the two vars below.
    key: process.env.PAYMENT_KEY || '',
    secret: process.env.PAYMENT_SECRET || '',
    stripeSecretKey: process.env.STRIPE_SECRET_KEY || '',
    stripeWebhookSecret: process.env.STRIPE_WEBHOOK_SECRET || '',
    // Where Stripe Checkout sends the browser back after success/cancel.
    // Defaults derived from FRONTEND_URL so a minimal .env doesn't need
    // to duplicate it.
...
function assertProductionConfigIsSafe() {
  ...
  if (env.payment.provider === 'dev-stub') {
    problems.push(
      'PAYMENT_PROVIDER=dev-stub cannot be used in production — set PAYMENT_PROVIDER=stripe ' +
        'with STRIPE_SECRET_KEY/STRIPE_WEBHOOK_SECRET (see .env.example)'
    );
  }
  if (env.payment.provider === 'stripe' && (!env.payment.stripeSecretKey || !env.payment.stripeWebhookSecret)) {
    problems.push('PAYMENT_PROVIDER=stripe requires STRIPE_SECRET_KEY and STRIPE_WEBHOOK_SECRET');
  }
