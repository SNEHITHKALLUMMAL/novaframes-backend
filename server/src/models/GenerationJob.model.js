...
const generationJobSchema = new Schema(
  {
    ...
    // The X-Request-Id of the HTTP request that created this job
    // (PHASE_17) — lets the worker's log lines for this job carry the
    // SAME requestId the original API call's logs used, so the whole
    // lifecycle (API → queue → worker) is one grep away, not two
    // separately-scoped ID spaces that have to be manually cross-referenced.
    requestId: {
      type: String,
      default: null,
    },
