...
const baseCookieOptions = {
  ...
  // Must be 'none' (not 'strict' or 'lax') in production because the frontend
  // (Vercel) and backend (Render) are on different origins. 'strict' blocks
  // cookies on ALL cross-site requests (including XHR/fetch), so the browser
  // never attaches the refresh-token cookie to the POST /auth/refresh call,
  // causing the 'Refresh token missing' error. 'none' allows cross-origin
  // cookie transmission when combined with secure:true (HTTPS). CSRF risk is
  // mitigated by the CORS configuration (specific origin + credentials:true).
