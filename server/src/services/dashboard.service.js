...
export async function getDashboardSummary(userId) {
  const [statusAgg, recentJobs, recentVideos, totalVideos] = await Promise.all([
    ...
    Video.find({ owner: userId })
      ...
      .select('title thumbnailUrl thumbnailKey status durationSeconds resolution type createdAt')
      .lean(),
    Video.countDocuments({ owner: userId }),
  ]);

  // Same reasoning as video.service.js/generation.service.js (PHASE_14) —
  // regenerate thumbnailUrl fresh rather than trusting the stored value.
  await Promise.all([
    ...recentJobs.map((job) => (job.outputVideo ? attachSignedUrls(job.outputVideo) : null)),
    ...recentVideos.map((video) => attachSignedUrls(video)),
  ]);
