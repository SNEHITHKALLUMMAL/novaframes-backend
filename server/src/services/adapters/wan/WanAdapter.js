 ...
 *
 * KNOWN LIMITATION (documented in full in ai-worker/README.md, not fixed
 * there or here): pythonBridge.js spawns a fresh Python process per job,
 * so the model is reloaded from disk on every single generation rather
 * than cached across jobs — a real, meaningful latency cost at real
 * usage volume. The fix (a persistent model-server process) is a
 * separate, larger, independently-testable piece of work.
 */
export class WanAdapter extends BaseVideoModel {
  ...
  async generate({ job, model, onProgress }) {
    ...
    let downloadedInputImagePath = null; // set only when getLocalCopy() is used (non-local storage)
    ...
    try {
      ...
      if (job.type !== 'text-to-video') {
        ...
        // Only local-disk paths belong to the app's own storage root and
        // must not be deleted; a downloaded temp copy (non-local storage)
        // is ours to clean up — tracked so the finally block below can do it.
        if (inputImagePath.startsWith(os.tmpdir())) {
          downloadedInputImagePath = inputImagePath;
        }
      ...
      if (downloadedInputImagePath) {
        await fs.rm(path.dirname(downloadedInputImagePath), { recursive: true, force: true });
      }
    }
  }
  ...
  async #resolveInputImagePath(job) {
    ...
    const storage = getStorageProvider();
    // LocalStorageProvider: the file already sits on the same disk the
    // worker runs on — just resolve its real path, no copy needed.
    if (typeof storage.getAbsolutePath === 'function') {
      return storage.getAbsolutePath(fileDoc.storageKey);
    }
    // Cloud storage backends (ObjectStorageProvider): download to a temp
    // file so the Python inference script has a real filesystem path to
    // read. generate()'s finally block cleans this up (see
    // downloadedInputImagePath there).
    if (typeof storage.getLocalCopy === 'function') {
      return storage.getLocalCopy(fileDoc.storageKey);
    }
    throw new Error(
      'Current StorageProvider cannot resolve a local file path for WanAdapter input — ' +
        'it must implement either getAbsolutePath() or getLocalCopy().'
    );
  }
}
